...
createRelMap :: [(Int, Int)] -> Int -> [Int] -> [(Int, [Int])] -> M.Map Int [Int]
createRelMap rels sup subs acc =
  case rels of
    [] | null subs -> M.fromList acc
    [] -> M.fromList $ (sup, subs) : acc
    (sup1, sub) : relt
      | sup1 == sup ->
        createRelMap relt sup (sub : subs) acc
    (sup1, sub) : relt ->
      createRelMap rels sup1 [] ((sup, subs) : acc)
